...
| `chaos.ts` | Process-level chaos primitives — currently `eventLoopPause(ms)`. |
...
| `scenarios/pause-during-authsync.stress.spec.ts` | Synchronous JS-runtime suspension via `eventLoopPause(ms)` (2 s, 10 s, 30 s) at key phases. Models "phone backgrounded mid-handshake". |

See [FINDINGS.md](./FINDINGS.md) for confirmed bugs and ongoing investigations.

## Chaos primitives

Two layers of chaos are available, composable in any `ChaosProfile`:

- **Network-layer (Toxiproxy):** `toxics`, `outages`, `flaps`. Latency,
  jitter, drops, slicing, kernel-level connection drops via proxy
  enable/disable.
- **Process-level (event-loop block):** `pauses`. Synchronous JS
  suspension at named phases via `eventLoopPause(ms)`. Models the
  "phone backgrounded mid-handshake" failure mode where the OS suspends
  the JS runtime entirely. See `chaos.ts` for the design choice (in-process
  block vs SIGSTOP) and honest limitations (kernel-side keepalive expiry
  is not modelled by pause alone — combine with an outage in the same
  phase to model that).

Canonical pause profiles in `CHAOS_PROFILES`:

- `pause-2s-pre-create` — 2 s suspension between connect and createCommunity.
- `pause-10s-during-authsync` — 10 s pause during the AUTH_SYNC handshake.
- `pause-30s-pre-create` — 30 s pause (took a phone call mid-flow).
- `pause-10s-with-outage-during-authsync` — combined: pause + simultaneous
  proxy outage so the kernel-side connection drop completes during the JS
  suspension.
