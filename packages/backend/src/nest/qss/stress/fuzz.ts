...
/**
 * Synchronous event-loop block applied at the named phase. Models the
 * "phone backgrounded mid-handshake" failure mode where iOS / Android
 * suspends the JS runtime entirely. See `chaos.ts` for the primitive and
 * its honest limitations vs OS-level SIGSTOP.
 */
export interface PauseWindow {
  /** When the pause starts (which phase). */
  at: ScenarioPhase
  /** Length of the JS-suspension in ms. */
  durationMs: number
}
...
export interface ChaosProfile {
  ...
  flaps?: FlapWindow[]
  /** Synchronous event-loop blocks applied at specific phases. */
  pauses?: PauseWindow[]
}
...
export const CHAOS_PROFILES: readonly ChaosProfile[] = [
  ...
  // ── Process-pause profiles (event-loop block) ───────────────────────
  // Models "phone backgrounded mid-handshake": JS pauses entirely, kernel
  // socket stays alive. See chaos.ts for the limitation discussion.
  {
  ...
  // Combined: pause + simultaneous outage models "phone backgrounded long
  // enough that the kernel TCP keepalive expires and the connection drops
  // before JS resumes". The outage is what produces the FIN/RST that JS
  // sees on resume.
  {
