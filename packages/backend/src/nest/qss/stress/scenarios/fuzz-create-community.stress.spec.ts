...
async function maybeOutage(
  ...
  // Pauses run first at each phase: model "phone went into background
  // right at this point". Synchronous block — nothing else can run until
  // it completes. If the same phase also has an outage, the outage starts
  // first so the kernel-side connection drop happens *during* the JS
  // suspension (matches "TCP keepalive expires while phone is asleep").
  for (const pause of (profile.pauses ?? []).filter(p => p.at === phase)) {
    await schedulePause(harness, profile, pause, phase)
  }
  // If pauses scheduled their own outages, skip them here so we don't
  // double-apply.
  const consumedOutages = new Set<OutageWindow>()
  for (const pause of (profile.pauses ?? []).filter(p => p.at === phase)) {
    for (const outage of (profile.outages ?? []).filter(o => o.at === phase)) {
      // Heuristic: if the outage at this phase has duration <= pause
      // duration, schedulePause already handled it.
      if (outage.durationMs <= pause.durationMs) consumedOutages.add(outage)
    }
  }
...
/**
 * Run a synchronous event-loop block at the named phase. If the same phase
 * also has an outage, disable the proxy first so the kernel-side connection
 * drop is in flight when JS resumes — that's the "TCP keepalive expires
 * while phone is backgrounded" path.
 */
async function schedulePause(
  ...
  const sameSlotOutage = (profile.outages ?? []).find(o => o.at === phase && o.durationMs <= pause.durationMs)
  if (sameSlotOutage != null) {
    await harness.toxiproxy.setEnabled(harness.proxyName, false).catch(() => undefined)
  }
  // Synchronous block — entire JS world freezes for this duration.
  eventLoopPause(pause.durationMs)
  if (sameSlotOutage != null) {
    await harness.toxiproxy.setEnabled(harness.proxyName, true).catch(() => undefined)
    // After pause + outage, drive an explicit reconnect so the scenario
    // doesn't have to wait QSS_RECONNECT_DELAY_MS (60 s).
